...
class Vault:
    ...
    def parse_ref(self, ref):
        ...
        # A shared folder path can be nested (`Engineering/Dev`), and the ref
        # pattern splits on the first slash, so a reference written with the
        # real folder name would otherwise parse as vault `Engineering`, item
        # `Dev` and be refused with advice to use a server that does not exist.
        # Recognise the real name before the regex gets a chance to cut it up.
        prefix = "keeper://%s/" % self.real_name
        if self.real_name and ref.startswith(prefix):
            ref = "keeper://%s/%s" % (self.alias, ref[len(prefix):])
